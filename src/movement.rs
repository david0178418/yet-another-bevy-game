...
type EnemyTransformQuery<'w, 's> = Query<'w, 's, (Entity, &'static Transform), (With<crate::behaviors::EnemyTag>, Without<crate::behaviors::MaintainDistance>)>;
...
fn update_seek_target_entities(
	mut seek_query: Query<(&mut crate::physics::Velocity, &crate::behaviors::SeekTarget, Has<crate::behaviors::Stunned>)>,
...
fn update_zigzag_entities(
	mut zigzag_query: Query<(&Transform, &mut crate::physics::Velocity, &mut crate::behaviors::ZigZagMovement, Has<crate::behaviors::Stunned>)>,
	player_query: Query<&Transform, (With<crate::behaviors::PlayerTag>, Without<crate::behaviors::ZigZagMovement>)>,
	...
	if let Ok(player_transform) = player_query.single() {
		for (transform, mut velocity, mut zigzag, is_stunned) in zigzag_query.iter_mut() {
			...
			let oscillation = (zigzag.time * zigzag.oscillation_speed).sin() * zigzag.oscillation_amplitude;

			let final_direction = (direction_to_player + perpendicular * oscillation).normalize_or_zero();
...
fn update_maintain_distance_entities(
	mut maintain_query: Query<(&Transform, &mut crate::physics::Velocity, &crate::behaviors::MaintainDistance, Has<crate::behaviors::Stunned>)>,
	player_query: Query<&Transform, (With<crate::behaviors::PlayerTag>, Without<crate::behaviors::MaintainDistance>)>,
